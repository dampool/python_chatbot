from tkinter import *
from py_chatbot_logic import chat
from py_chatbot_logic import normalResponse
from py_chatbot_logic import user_name
...
class ChatInterface(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.text_box = Text(self.text_frame, yscrollcommand=self.text_box_scrollbar.set,
                             ...
                             width=10, height=1)
        username = "PyBot: "+user_name +"\n"
        name = self.text_box.insert(END,username)
        ...
        # send button
        self.send_button = Button(self.send_button_frame, text="Send",  width=5, relief=GROOVE, bg='green',
                                  bd=1, command=lambda: self.send_message_insert(self.nameflag), activebackground="light green",
    ...
    def send_message_insert(self, message):
        #namme = ""
        if message == False:
            self.namme = self.entry_field.get()
            self.entry_field.delete(0, END)
            self.nameflag = True
            res = "PyBot: " + normalResponse + self.namme+ ", How can I help you?"+"\n"
            self.text_box.insert(END,res)
            return  self.namme


        else:
            user_input = self.entry_field.get()
            pr1 = self.namme +" :"+ user_input + "\n"
